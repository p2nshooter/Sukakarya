...
--
-- Every row below is invented: the news, the businesses, the tourist sites,
-- the population figures, the budget lines, the events, the galleries and the
-- announcements. None of it describes any real village.
--
-- It exists so a *demonstration* deployment renders a full site instead of a
-- grid of empty states, and so a village evaluating the CMS can see what each
-- section looks like with content in it.
--
-- It is NOT applied to a live village. The bootstrap workflow runs it only when
-- the `demo_content` input is ticked, because publishing an invented
-- musyawarah, an invented APBDes figure or an invented population count on a
-- real village's official site states things that did not happen.
--
-- Real, generically-true content lives in base-content.sql and is always
-- applied. There is deliberately no personal data of any kind here - no NIK, no
-- KK number, no home address, no private telephone number.
--
